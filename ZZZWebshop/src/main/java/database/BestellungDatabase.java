...
public class BestellungDatabase {
	...
	public static boolean fuegeBestellung(Warenkorb warenkorb, String zahlungsmethode, int bestellid) {
		...
		try {
			...
			PreparedStatement pstmt = con.prepareStatement(
					"INSERT INTO bestellungen (nummer, benutzerid, artikelid, menge, zahlungsmethode) VALUES(?,?,?,?,?)");
	...
	public static int hoechsteBestellid(int benutzerId) {
		...
		try {
			...
			PreparedStatement pstmt = con
					.prepareStatement("SELECT id FROM bestellungen WHERE benutzerid = ? ORDER BY id DESC LIMIT(1)");
	...
	/**
	 * Diese Methode gibt alle Bestellungen innerhalb einer grossen Bestellung mit
	 * der selben Bestellid eines genauen Benutzers als Liste zurueck.
	 * 
	 * @param benutzerId Identifikation Benutezr.
	 * @param bestellId  Bestellungen mit der selben Id bilden eine gesamte
	 *                   Bestellung.
	...
	public static ArrayList<Bestellung> getBestellungen(int benutzerId, int bestellId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			PreparedStatement pstmt = con.prepareStatement(
					"SELECT bestellungen.id, bestellungen.menge, bestellungen.bestellt_am, artikel.name, artikel.preis "
	...
	/**
	 * Gibt bestellids aller Bestellungen eines Benutzers zurueck, um alle
	 * Bestellungen zu ermitteln.
	 * 
	...
	public static List<Integer> getBestellnummern(int benutzerId) {
		...
		try {
			con = DatabaseConnection.getConnection();
			PreparedStatement pstmt = con.prepareStatement(
					"SELECT DISTINCT nummer from bestellungen WHERE benutzerid = ? order by nummer DESC");
			pstmt.setInt(1, benutzerId);
			ResultSet rs = pstmt.executeQuery();
