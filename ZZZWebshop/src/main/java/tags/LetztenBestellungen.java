...
public class LetztenBestellungen extends SimpleTagSupport {
	...
	/**
	 * Aus der Datenbank werden alle Bestellungen eines Nutzers geholt. Dann werden
	 * alle Bestellnummern einzelnen aus der Datenbank geholt, um die
	 * Gesamtbestellungen zu ermitteln. Anhand der Bestellnummern koennen wir uns
	 * die genauen Bestellungen mit der gleichen Bestellnummer zusammen in einer
	 * Liste abspeichern. Diese Bestellungen werden dann visuell als Art Liste fuer
	...
	public void doTag() throws JspException, IOException {
		...
		if (bestellnummern.size() != 0) {
			...
			// .multicollape Klasse hinzuzufügen
			int i = 1;
			...
			for (ArrayList<Bestellung> bestellungen : alleBestellungen) {
				...
				out.print("	<div class=\"accordion-item " + mehrAnzeigen + "\" id=\"accordion\">"
						...
						+ String.format("%.02f", gesamtpreis) + " &euro;</div>" + "				</div>"
				...
				for (Bestellung bs : bestellungen) {
					out.print("			<div class=\"card my-2\">" + "				<div class=\"card-body py-2\">"
							+ "					<div class=\"row\">"
							+ "						<div class=\"col col-6\">"
							+ "							<p class=\"card-text\">" + bs.getName() + "</p>"
							+ "						</div>" + "						<div class=\"col col-4\">"
							+ "							<p class=\"card-text\">" + bs.getMenge() + " x "
							+ String.format("%.02f", bs.getPreis()) + " &euro; </p>" + "						</div>"
							+ "						<div class=\"col col-2\">"
							+ "							<h6 class=\"card-title\">"
							+ String.format("%.02f", bs.getMenge() * bs.getPreis()) + " &euro;</h6>"
							+ "						</div>" + "					</div>" + "				</div>"
							+ "			</div>");
				}
